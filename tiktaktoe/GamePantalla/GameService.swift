...
class GameService: ObservableObject {
    ...
    @Published var possiblMoves = Move.all
    ...
    func makeMove(at index: Int) {
        if gameBoard[index].player == nil{
            ...
            if let matchingIndex = possiblMoves.firstIndex(where: {$0 == (index + 1)}){
                possiblMoves.remove(at: matchingIndex)
            }
            toggleCurrent()
        }
        if possiblMoves.isEmpty {
            gameOver = true
        }
    }
}
